...
import { fetchOutputTaxLedger, createOutputTaxEntry } from '../../services/taxAPI';

const fmt = (n) => new Intl.NumberFormat('en-IN', { style:'currency', currency:'INR', maximumFractionDigits:0 }).format(n||0);
const ENTRY_TYPES = ['liability','credit_note','reversal','payment','opening','closing'];
const TAX_HEADS   = ['igst','cgst','sgst','cess'];
const EMPTY = { entryType:'liability', taxHead:'igst', period:'', amount:0, referenceNumber:'', narration:'' };

export default function AdminOutputTax() {
  ...
  const load = useCallback(async () => {
    try {
      const r = await fetchOutputTaxLedger({ page, limit:20, ...filter });
      setRows(r.data.data || []);
  ...
  const save = async () => {
    setSaving(true);
    try { await createOutputTaxEntry(form); await load(); setModal(false); setForm(EMPTY); }
    catch(e) { alert(e.response?.data?.message||'Error'); }
    finally { setSaving(false); }
  };

  const typeColor = t => ({ liability:'#e74c3c', credit_note:'#27ae60', reversal:'#f39c12', payment:'#3498db', opening:'#9b59b6', closing:'#888' }[t]||'#888');

  return (
    <div style={{ padding:'24px 32px', fontFamily:'Poppins, sans-serif' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24 }}>
        <h2 style={{ margin:0, fontSize:22, fontWeight:700, color:'#1a1a2e' }}>Output Tax Ledger</h2>
          ...
          <tbody>
            {rows.map(r=>(
              <tr key={r._id} style={{ borderTop:'1px solid #f5f5f5' }}>
                ...
                <td style={{ padding:'11px 16px', fontWeight:600, color:r.entryType==='liability'?'#e74c3c':'#27ae60' }}>{fmt(r.amount)}</td>
                <td style={{ padding:'11px 16px' }}>{fmt(r.runningBalance)}</td>
                <td style={{ padding:'11px 16px', color:'#888', fontSize:12 }}>{r.narration||'—'}</td>
              </tr>
            ))}
            {rows.length===0&&<tr><td colSpan={7} style={{ padding:32, textAlign:'center', color:'#aaa' }}>No output tax ledger entries.</td></tr>}
          </tbody>
        </table>
        {pages>1 && (
      ...
      {modal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
          <div style={{ background:'#fff', borderRadius:16, padding:32, width:440, maxWidth:'95vw', boxShadow:'0 8px 32px rgba(0,0,0,0.18)' }}>
            <h3 style={{ margin:'0 0 24px', fontWeight:700 }}>Add Output Tax Entry</h3>
