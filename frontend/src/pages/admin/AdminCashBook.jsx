...
import { fetchCashBook } from '../../services/financeAPI';
...
export default function AdminCashBook() {
  ...
  const load = async (p = 1) => {
    if (!accountId) return alert('Enter a Cash Account ID');
    setLoading(true); setError(''); setData(null);
    try {
      const res = await fetchCashBook({ accountId, startDate, endDate, page: p, limit: LIMIT });
  ...
  return (
    <div className="p-6 space-y-6">
      <SectionHeader title="Cash Book" subtitle="Cash account transactions" />

      <div className="bg-white rounded-xl shadow-sm p-5 flex flex-wrap gap-4 items-end">
        <div><label className="text-xs text-gray-500 block mb-1">Cash Account ID *</label><input required value={accountId} onChange={e=>setAccId(e.target.value)} placeholder="Account ID" className="border rounded-lg px-3 py-2 text-sm" /></div>
